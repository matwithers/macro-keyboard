...
def event(device, keycode, value=""):
    if device == DELAY:
        ...
        return device.value + str(keycode) + "i" + str(value)
    elif type(value) == ActionCode:
        return device.value + str(keycode) + value.value
    else:
        return device.value + str(keycode)
...
class HIDKeyCode:
    ...
    def buildKeyStrokes(self, key_string):
        key_events = []
        print("-----------------------------------------")
        print("initial key_string: " + key_string)
        key_sequ = key_string.split("|")
        print("sequence length: " + str(len(key_sequ)))

        for i in range(len(key_sequ)):
            print("  sequence " + str(i) + ": " + key_sequ[i])

            stroke_combination = key_sequ[i].split("+")
            for j in range(len(stroke_combination)):
                print("    stroke " + str(j) + ": " + stroke_combination[j])

                if self.getKeyCode(stroke_combination[j]):
                    key_events.append(event(DeviceCode.KEYBOARD, self.getKeyCode(stroke_combination[j]), ActionCode.PRESS))
                    print("    key_events.append(event(DeviceCode.KEYBOARD, " + str(self.getKeyCode(stroke_combination[j])) + " , ActionCode.PRESS)))")

            for j in range((len(stroke_combination)), 0, -1):
                print("    stroke " + str(j-1) + ": " + stroke_combination[j-1])

                if self.getKeyCode(stroke_combination[j-1]):
                    key_events.append(event(DeviceCode.KEYBOARD, self.getKeyCode(stroke_combination[j-1]), ActionCode.RELEASE))
                    print("    key_events.append(event(DeviceCode.KEYBOARD, " + str(self.getKeyCode(stroke_combination[j-1])) + " , ActionCode.RELEASE)))")
